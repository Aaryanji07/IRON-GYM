  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.wrap} className="fade-in">
        <Logo size={38}/>
        <div style={s.card}>
          <div style={s.roleBadge}>CLIENT</div>
          <h1 style={s.title}>Join IronFit</h1>
          <p style={s.sub}>Create your member account and start booking</p>
          {error && <div style={s.err}>{error}</div>}
          <form onSubmit={submit} style={s.form}>
            <div style={s.field}>
              <label style={s.lbl}>Full Name</label>
              <input type="text" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Alex Johnson" required style={s.inp}
                onFocus={e=>e.target.style.borderColor="var(--accent)"} onBlur={e=>e.target.style.borderColor="var(--border)"}/>
            </div>
            <div style={s.field}>
              <label style={s.lbl}>Email</label>
              <input type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} placeholder="you@example.com" required style={s.inp}
                onFocus={e=>e.target.style.borderColor="var(--accent)"} onBlur={e=>e.target.style.borderColor="var(--border)"}/>
            </div>
            <div style={s.field}>
              <label style={s.lbl}>Phone (optional)</label>
              <input type="tel" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} placeholder="+91 99999 00000" style={s.inp}
                onFocus={e=>e.target.style.borderColor="var(--accent)"} onBlur={e=>e.target.style.borderColor="var(--border)"}/>
            </div>
            <div style={s.field}>
              <label style={s.lbl}>Password</label>
              <input type="password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})} placeholder="Min. 6 characters" required style={s.inp}
                onFocus={e=>e.target.style.borderColor="var(--accent)"} onBlur={e=>e.target.style.borderColor="var(--border)"}/>
            </div>
          ...
          <p style={s.foot}>Already have an account? <Link to="/login" style={{color:"var(--accent)",fontWeight:500}}>Sign In</Link></p>
...
const s = {
  ...
  bg:{position:"fixed",inset:0,background:"radial-gradient(ellipse at 60% 30%, rgba(232,255,71,.04) 0%,transparent 60%)",pointerEvents:"none"},
  ...
  roleBadge:{display:"inline-block",fontSize:11,fontWeight:700,padding:"4px 12px",borderRadius:20,letterSpacing:1,marginBottom:14,textTransform:"uppercase",background:"rgba(232,255,71,.1)",color:"var(--accent)",border:"1px solid rgba(232,255,71,.25)"},
  ...
  btn:{padding:"14px",background:"var(--accent)",color:"var(--black)",border:"none",borderRadius:8,fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:"inherit",marginTop:4},
